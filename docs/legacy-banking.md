# Legacy banking algorithm (rescue copy)

This file preserves the fork's macOS SceneKit camera-banking implementation so it can be ported to
`crates/gibson-scene` after the legacy `src/` tree is deleted. It is reference material for the
Batch 1 scene agent — not a cleanup doc.

## Provenance

- File: `src/scenekit/GibsonSCNCamera.mm`
- Commit: `797bb16` (and earlier), deleted in the Scaffold batch (Batch 0) of the Rust modernization.
- Method: `-[GibsonSCNCamera updateAtTime:startTime:]`, **lines 138–193** (verbatim below).
- Helpers it depends on (same file, lines 18–92, also verbatim below): `gibson_clamp_wrap`,
  `gibson_spline_at`, `gibson_v`, `gibson_safe_norm`, and `-[GibsonSCNCamera aimWithForward:up:position:]`
  (lines 125–136). `gibson_fly_speed()` / `gibson_bank_max_degrees()` / `gibson_bank_strength()` /
  `gibson_bank_smoothing()` come from `src/gibson_settings.cpp` (defaults: speed 0.55, bank strength
  0.45, bank max degrees 32.0 — never parsed in the legacy app, bank smoothing 0.55).
- Constants come from `src/fly_path.h`: `GIBSON_FLY_SPEED 0.55f`, `GIBSON_FLY_TIGHTNESS 0.5f`,
  `GIBSON_FLY_LOOKAHEAD_MS 150`.
- Note: `gibson_spline_at` in the fork is a **ping-pong** (alternating-direction) Catmull-Rom over 40
  spans of a 41-row control list whose last two rows duplicate row 0. The modernized spec replaces it
  with a **closed wrapping Catmull-Rom over the 39 unique waypoints** (see
  `crates/gibson-scene/src/flight_path.rs`), so the port should keep the banking math below but
  substitute `position(s)` from the new `FlightPath`.

## `updateAtTime:startTime:` (lines 138–193)
...
Port notes (behavior to preserve exactly):

- Frame dt is clamped to `[1/120, 0.1]` s; the first frame assumes `1/60` s.
- Yaw rate is estimated from spline tangent directions `eps = 0.14` segments apart; tangents that are
  ~parallel to `forward` (or degenerate) fall back to `forward` via `gibson_safe_norm`.
- `signedYaw` magnitudes over `0.45` are treated as wrap/seam artifacts and zeroed.
- Yaw-rate low-pass: `τ = 0.28 s` (`rateFollow = 1 − exp(−dt/0.28)`).
- Target bank `= −yawRate × bank_strength`, clamped to `±bank_max_degrees`.
- Bank low-pass: `τ = max(0.12, bank_smoothing)` s.
- Roll applies only when `|bank| > 1e-4`; if the rolled up vector drops below `0.15` dot with world
  up, world up is restored.

## `aimWithForward:up:position:` (lines 125–136)
